...
def make_fake_control(pc, mobile, smartphone):
    control = mock.Mock("*FakePageQueryControl*")
    def fake_static_pageset_query(request, url, dt, page_type):
        class ModelPage:
            @classmethod
            def first(self):
                return page_type
...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self._setup_component(self.config)

    def _setup_component(self, config):
        ...
        self.control = control = make_fake_control(S.pc, S.mobile, S.smartphone)
