...
def page_accesskey_section_panel(context, request, page):
    ...
    return dict(page=page, page_title=u"アクセスキー", 
                preview_with_accesskey_url_gen=lambda hashkey, organization_code: request.route_url("preview_page", page_id=page.id, _query=dict(access_key=hashkey, organization_code = organization_code)),
