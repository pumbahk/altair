...
def unpublish_files_on_s3(after_s3_delete):
    ...
    logger.info("*debug unpublish_deleted_files_on_s3 start. assets={0}".format(assets))
    for asset in assets:
        for filename in asset.all_files_candidates():
            filename = make_s3_upload_filename(request, filename)
            logger.info("*debug unpublish key name={0}".format(filename))
...
def publish_files_on_s3(after_s3_delete):
    ...
    assets = event.extra_args
    uploader = event.uploader
    logger.info("*debug publish_files_on_s3 start. assets={0}".format(assets))
    for asset in assets:
        for filename in asset.all_files_candidates():
            filename = make_s3_upload_filename(request, filename)
            logger.info("*debug publish key name={0}".format(filename))
...
def set_file_url(after_s3_upload):
    ...
    assets = event.extra_args
    logger.info("*debug set_file_url start. assets={0}".format(assets))
    ...
    for k, D in s3uploaded_dict.items():
        ...
        if asset:
            ...
            logger.info("*debug file url is set. asset={2} file_url={0} thumbnail_url={1}".format(D.get("file_url"), D.get("thumbnail_url"), asset))
