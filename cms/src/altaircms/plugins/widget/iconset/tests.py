...
class IconsetWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    def setUp(self):
        self.config = config
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.iconset.views import IconsetWidgetView
        from altaircms.plugins.widget.iconset.models import IconsetWidgetResource
        request.context = IconsetWidgetResource(request)
        return IconsetWidgetView(request)

    def test_create(self):
        """ test creating IconsetWidget.
        check:
          1. Iconset Widget is created,  successfully.
          2. Iconset Widget and Page object is bounded
        """
        ...
        created = IconsetWidget.query.one()
        self.assertEquals(IconsetWidget.query.count(), 1)
    ...
    def test_update(self):
        ...
        updated = IconsetWidget.query.one()
        self.assertEquals(IconsetWidget.query.count(), 1)
    ...
    def test_delete(self):
        ...
        self.assertEquals(IconsetWidget.query.count(), 0)
