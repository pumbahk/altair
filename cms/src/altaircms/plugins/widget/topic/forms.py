...
class TopicChoiceForm(form.Form):
    kind = fields.SelectField(id="kind", label=u"トピックの種別", choices=[(x, x) for x in Topic.TYPE_CANDIDATES])
    display_count = fields.IntegerField(id="display_count", label=u"表示件数", default=5, validators=[validators.Required()])
    display_global = fields.BooleanField(id="display_globals", label=u"グローバルトピックを表示", default=True)
