...
class WidgetTestSourceMixn(object):
    ...
    def _makeAsset(self, id=None):
        ...
        return MovieAsset.from_dict({"id": id, "filepath": "test.jpg"})
...
class MovieWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.movie.views import MovieWidgetView
        from altaircms.plugins.widget.movie.models import MovieWidgetResource
        request.context = MovieWidgetResource(request)
        return MovieWidgetView(request)
