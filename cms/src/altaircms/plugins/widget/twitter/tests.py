...
class TwitterWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    def setUp(self):
        self.config = config
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.twitter.views import TwitterWidgetView
        from altaircms.plugins.widget.twitter.models import TwitterWidgetResource
        request.context = TwitterWidgetResource(request)
        return TwitterWidgetView(request)

    def test_create(self):
        """ test creating TwitterWidget.
        check:
          1. Twitter Widget is created,  successfully.
          2. Twitter Widget and Page object is bounded
        """
        ...
        created = TwitterWidget.query.one()
        self.assertEquals(TwitterWidget.query.count(), 1)
    ...
    def test_update(self):
        ...
        updated = TwitterWidget.query.one()
        self.assertEquals(TwitterWidget.query.count(), 1)
    ...
    def test_delete(self):
        ...
        self.assertEquals(TwitterWidget.query.count(), 0)
