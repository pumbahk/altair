...
class RawhtmlWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    def setUp(self):
        self.config = config
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.rawhtml.views import RawhtmlWidgetView
        from altaircms.plugins.widget.rawhtml.models import RawhtmlWidgetResource
        request.context = RawhtmlWidgetResource(request)
        return RawhtmlWidgetView(request)

    def test_create(self):
        """ test creating RawhtmlWidget.
        check:
          1. Rawhtml Widget is created,  successfully.
          2. Rawhtml Widget and Page object is bounded
        """
        ...
        created = RawhtmlWidget.query.one()
        self.assertEquals(RawhtmlWidget.query.count(), 1)
    ...
    def test_update(self):
        ...
        updated = RawhtmlWidget.query.one()
        self.assertEquals(RawhtmlWidget.query.count(), 1)
    ...
    def test_delete(self):
        ...
        self.assertEquals(RawhtmlWidget.query.count(), 0)
