...
class PurchaseWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    def setUp(self):
        self.config = config
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.purchase.views import PurchaseWidgetView
        from altaircms.plugins.widget.purchase.models import PurchaseWidgetResource
        request.context = PurchaseWidgetResource(request)
        return PurchaseWidgetView(request)

    def test_create(self):
        """ test creating PurchaseWidget.
        check:
          1. Purchase Widget is created,  successfully.
          2. Purchase Widget and Page object is bounded
        """
        ...
        created = PurchaseWidget.query.one()
        self.assertEquals(PurchaseWidget.query.count(), 1)
    ...
    def test_update(self):
        ...
        updated = PurchaseWidget.query.one()
        self.assertEquals(PurchaseWidget.query.count(), 1)
    ...
    def test_delete(self):
        ...
        self.assertEquals(PurchaseWidget.query.count(), 0)
