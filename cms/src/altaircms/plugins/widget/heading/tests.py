...
class HeadingWidgetViewTests(WidgetTestSourceMixn, 
                          unittest.TestCase):
    def setUp(self):
        self.config = config
    ...
    def _makeTarget(self, request):
        from altaircms.plugins.widget.heading.views import HeadingWidgetView
        from altaircms.plugins.widget.heading.models import HeadingWidgetResource
        request.context = HeadingWidgetResource(request)
        return HeadingWidgetView(request)

    def test_create(self):
        """ test creating HeadingWidget.
        check:
          1. Heading Widget is created,  successfully.
          2. Heading Widget and Page object is bounded
        """
        ...
        created = HeadingWidget.query.one()
        self.assertEquals(HeadingWidget.query.count(), 1)
    ...
    def test_update(self):
        ...
        updated = HeadingWidget.query.one()
        self.assertEquals(HeadingWidget.query.count(), 1)
    ...
    def test_delete(self):
        ...
        self.assertEquals(HeadingWidget.query.count(), 0)
