...
class Bj89ersFixtureBuilder(FixtureBuilder):
    def __init__(self, Datum):
        ...
        topic_items: (display_order, kind, subkind, title, text, )
        """
        ...
        topic_items = [
            ...
            (3, u"89ers質問", u"購入について", u"ナイナーズチケットでの決済方法はどうすれば良いですか？", u"クレジットカード決済(通常のカード決済あるいは「楽天ID決済」でのカード決済)とセブン-イレブン店頭でのお支払が可能です。 購入手続きと同時に、ご指定の決済方法で代金を決済していただきます。なお、支払方法・引取方法により手数料がかかる場合があります 。", u'89ers:よくある質問'),
