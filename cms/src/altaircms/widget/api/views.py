    ...
    def create_widget_detail(self, structure_dict):
        for block in structure_dict:
            for widget in structure_dict[block]:
                ...
                if widget['name'] == u'image' and "pk" in widget:
                    target_widget = self.request.allowable(ImageWidget).filter(ImageWidget.id==widget['pk']).first()
                    if target_widget:
                        widget['detail'] = "<img style='height:40px' src='{0}' />".format(helpers.asset.rendering_object(self.request,target_widget.asset).thumbnail_path)
