...
%if picks.count() > 0:
  ...
  %for pick in picks:
	<img src="${h.asset.to_show_page(request,pick.image_asset)}" align="left" ${'width="%s"'% pick.image_asset.width if pick.image_asset.width else ""|} ${'height="%s"'% pick.image_asset.height if pick.image_asset.height else ""|}>
