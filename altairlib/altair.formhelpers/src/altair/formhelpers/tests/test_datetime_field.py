    ...
    def test_rest_of_fields_missing(self):
        ...
        for combination in itertools.product(*missing_value_defaults_combinations):
            ...
            for n in range(0, len(field_values)):
                ...
                field.process(WebobInputWrapper(MultiDict(formdata)))
                self.assertEqual(field.data, expected)
                self.assertEqual(field.raw_data, expected and [expected.strftime('%H:%M:%S')])

    def test_some_fields_missing(self):
        field = self._makeOne()
        field.process(WebobInputWrapper(MultiDict({'test.minute': '1'})))
        self.assertEqual(["Required field `hour' is not supplied"], field.process_errors)
        self.assertEqual(field.data, None)
        self.assertEqual(field.raw_data, None)

    def test_invalid_value(self):
        field = self._makeOne()
        field.process(WebobInputWrapper(MultiDict({'test.hour': 'XXXX'})))
        self.assertEqual(["Invalid value for hour", 'Not a valid time value'], field.process_errors)
        self.assertEqual(field.data, None)
        self.assertEqual(field.raw_data, None)
