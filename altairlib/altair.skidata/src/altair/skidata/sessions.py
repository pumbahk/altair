...
class SkidataWebServiceSession(object):
    ...
    def make_header(self, header_id):
        return Header(version=self._version, issuer=self._issuer, receiver=self._receiver, header_id=header_id)

    def send(self, event_ts_property=None, whitelist=None, blacklist=None, header_id=None, strict=True):
        """
        ...
        :param header_id: Unique ID (integer) for XML-Data generated by TS, recorded in HSH in case of error if set
        :param strict: boolean to send a request XML-Data with the values cut off at the maximum length.
        """
        header = self.make_header(header_id=header_id)
        ...
        try:
            ...
            if header_id is not None:
                msg = u'Failed to send a request to HSH (header ID: {}). reason: {}'.format(header_id, e)
            else:
                msg = u'Failed to send a request to HSH. reason: {}'.format(e)
            raise SkidataWebServiceError(msg)
