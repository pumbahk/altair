...
class SQLBuilder(object):
    ...
    def put_scalar(self, scalar):
        if isinstance(scalar, basestring):
            return u"'%s'" % scalar.replace(u"'", u"''")
        elif isinstance(scalar, datetime.datetime):
            return u"'%s'" % scalar.strftime("%Y-%m-%d %H:%M:%S")
        elif isinstance(scalar, datetime.date):
            return u"'%s'" % scalar.strftime("%Y-%m-%d")
    ...
    def insert(self, table, values):
        values = list(values)
        self.out.write("INSERT INTO %s (%s) VALUES (" % (table.encode(self.encoding), ', '.join(k.encode(self.encoding) for k, v in values)))
        first = True
        for k, v in values:
            if not first:
                self.out.write(", ")
            self.out.write(self.put_scalar(v).encode(self.encoding))
            first = False
        self.out.write(");\n");
