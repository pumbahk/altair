...
def main(argv=sys.argv):
    ...
    result_file = open("{0}_result.csv".format(lot_id), 'w')
    writer = csv.writer(result_file, lineterminator='\n')

    with open("{0}.csv".format(lot_id), 'r') as f:
        reader = csv.reader(f)
        order_list = list()
        first_row = None
        keep_order_no = None
        ...
        for row in reader:
            ...
            if keep_order_no != row[1]:
                write_formatted_content(writer, order_list, lot_id)
...
def write_formatted_content(writer, order_list, lot_id):
    '''
    ...
    out_list.append(lot_id)

    return writer.writerow(out_list)
