...
from ticketing.core.models import Event, SalesSegment, SalesSegmentGroup, Product
from ticketing.events.payment_delivery_method_pairs.forms import PaymentDeliveryMethodPairForm
from ticketing.events.sales_segment_groups.forms import SalesSegmentGroupForm, MemberGroupToSalesSegmentForm
...
class SalesSegmentGroups(BaseView):
    ...
    @view_config(route_name='sales_segment_groups.index', renderer='ticketing:templates/sales_segment_groups/index.html')
    def index(self):
    ...
    def _edit_post(self):
        ...
        if self.request.matched_route.name == 'sales_segment_groups.copy':
            ...
            if f.copy_products.data:
                for product in sales_segment_group.product:
                    Product.create_from_template(template=product, with_product_items=True, stock_holder_id=f.copy_to_stock_holder.data, sales_segment_group=id_map)
        else:
    ...
    @view_config(route_name='sales_segment_groups.copy', request_method='POST', renderer='ticketing:templates/sales_segment_groups/_form.html')
    @view_config(route_name='sales_segment_groups.edit', request_method='POST', renderer='ticketing:templates/sales_segment_groups/_form.html')
