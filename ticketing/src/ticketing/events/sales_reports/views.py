    ...
    def send_mail(self):
        ...
        if form.validate():
            if performance_id:
                ...
                html = render_to_response('ticketing:templates/sales_reports/performance_mail.html', render_param, request=self.request)
            elif event_id:
                render_param = {
                    'event_reporter':EventReporter(form, event)
                }
                html = render_to_response('ticketing:templates/sales_reports/event_mail.html', render_param, request=self.request)
