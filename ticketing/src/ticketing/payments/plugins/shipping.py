...
@view_config(context=ICartDelivery, name="delivery-1", renderer="ticketing.payments.plugins:templates/shipping_confirm.html")
def deliver_confirm_viewlet(context, request):
    logger.debug(u"郵送")
    cart = context.cart
    return dict(shipping_address=cart.shipping_address)

@view_config(context=IOrderDelivery, name="delivery-1", renderer="ticketing.payments.plugins:templates/shipping_confirm.html")
