...
def _role_id_list_to_role_list(role_id_list):
    return [ session.query(OperatorRole).filter(OperatorRole.id==role_id).one() for role_id in role_id_list]

@view_config(route_name='admin.operators.index', renderer='ticketing:templates/operators/index.html')
...
@view_config(route_name='admin.operators.new', renderer='ticketing:templates/operators/new.html')
def new(context, request):
    ...
    f = Form(
        ...
        buttons=(Button(name='submit',title=u'新規'),)
    )
...
@view_config(route_name='admin.operators.edit', renderer='ticketing:templates/operators/edit.html')
...
@view_config(route_name='admin.operators.show', renderer='ticketing:templates/operators/show.html')
