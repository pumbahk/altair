...
class OrderFormSchema(Form):
    ...
    tel_1 = fields.TextField(u"電話番号(携帯)", filters=[strip_spaces], validators=[v.Length(max=12), v.Regexp(r'^\d*$', message=u'-を抜いた数字のみを入力してください')])
    tel_2 = fields.TextField(u"電話番号(自宅)", filters=[strip_spaces], validators=[v.Length(max=12), v.Regexp(r'^\d*$', message=u'-を抜いた数字のみを入力してください')])
