...
7 fs
14 22 m 90 10 "主催: ブルーマングループ IN 、LLP<br/>お問合せ: ブルーマングループ公演事務局 03-5414-3255<br/>後援: 東京都、米国大使館、港区、東京メトロ<br/>営利目的の転売禁止<br/>4歳以下入場不可" X
6.35 fs
    ...
    def finish(self, request, cart):
        ...
        if not sej_order:
            request_order(
                ...
                commission_fee      = order.system_fee + order.transaction_fee_amount,
                payment_type        = SejPaymentType.PrepaymentOnly,
                ticketing_fee       = order.delivery_fee_amount,
