...
:f15 hc 1 S
12 fs :b hc 14 0 m 93 12 "BLUE MAN GROUP IN TOKYO" X pc 10 fs
...
7 fs
14 22 m 90 10 "主催: ブルーマングループ IN 東京、LLP<br/>お問合せ: ブルーマングループ東京公演事務局 03-5414-3255<br/>後援: 東京都、米国大使館、港区、東京メトロ<br/>営利目的の転売禁止<br/>4歳以下入場不可" X
6.35 fs
:b hc 13.3 42.75 m 30 2 "予約番号：" X 25.1 42.75 m 30 2 "0509-1234" X pc
:b hc 13.3 45.25 m 80 2 "お問合せ先：楽天チケット お問い合わせセンター 0000-0000-0000" X pc
8 fs
:b hc 112 5 m 28 8 "BLUE MAN GROUP IN TOKYO" X pc
112 14 m 28 12 "開催日 2012年<br/>7月9日(土)<br/>開場 12:30<br/>開演 13:00<br/>" X
112 32 m 28 15 "ポンチョ席<br/>1列%(seat_number)s番<br/>\\\\%(ticket_price)s(税込)" X
:b hc 7 fs 112 50 m 28 4 "0509-1234" X :pc
]]></b>
<FIXTAG01></FIXTAG01>
<FIXTAG02></FIXTAG02>
<FIXTAG03></FIXTAG03>
...
#            continue
        ...
        for i in range(0,ticket_num):
            ticket = dict(
                ...
                xml = SejTicketDataXml(ticket_date_xml(u'テスト:%s' % order_number, '%s'%ticket_total, order_id, number, seat_number))
            )
        ...
        for i in range(0, e_ticket_num):
            ticket = dict(
                ...
                ticket_template_id  = u'TTTS000001',
                performance_datetime= datetime.datetime(2012,8,31,18,00),
                xml = SejTicketDataXml(ticket_date_xml(u'テスト:%s' % order_number, '-', order_id, number, ''))
            )
            tickets.append(ticket)
