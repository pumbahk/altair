...
def load_tsv_file():
    ...
    csv_file = open('/Users/mistat/Desktop/xmls/3.csv')
    order_id = 1000
    seat_number = 1
    ri = 0
    for row in list(csv.reader(csv_file, dialect='excel'))[2:]:
        ...
        if len(cols) >= 17 and cols[16] != u'':
            ...
            ticket_total    = row[13].split(u'→')[0]
            commission_fee  = row[14].split(u'→')[0]
            ticketing_fee   = row[15].split(u'→')[0]
            total = row[16].split(u'→')[0]
            ...
            payment_due_at    = parse(row[18])
            ticketing_due_at  = parse(row[19])
            ticketing_start_at= parse(row[20].split(u'→')[0])
