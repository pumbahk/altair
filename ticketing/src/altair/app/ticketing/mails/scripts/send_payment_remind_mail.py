...
def get_target_order_nos(today, skip_already_notified=True):
    ...
    q = DBSession.query(Order)\
        ...
        .join(PaymentMethod)\
        .filter(or_(and_(PaymentMethod.payment_plugin_id == SEJ_PAYMENT_PLUGIN_ID,  Order.order_no.in_(sej_order_no)),
                and_(PaymentMethod.payment_plugin_id == FAMIPORT_PAYMENT_PLUGIN_ID,  Order.order_no.in_(famiport_order_no))))\
