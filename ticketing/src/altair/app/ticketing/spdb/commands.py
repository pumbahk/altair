...
class FileOperator(object):
    ...
    def upload(self, remote_path="/home/odin_ticket/from_ticket/", old=None):
        ...
        if not self._args.local:
            ...
            if old:
                ssh.connect(self.spdb_info.old_host, self.spdb_info.port, self.spdb_info.user,
...
class SpdbInfo(object):
    ...
    @property
    def old_host(self):
        return self.settings['spdb.old_host']
...
def send_spdb_data_old(args, connection):
    sql_creater = SQLCreater(args, spdb_old_sql)
    file_operator = FileOperator(args, SpdbInfo(args))

    file_operator.open_tmp_file()
    logger.info(u"SPDB old start {0}".format(file_operator.file_name))
    ...
    try:
        ...
        file_operator.write_tmp_file(
            u"\"PrimaryKey\",\"予約番号\",\"ステータス\",\"支払いステータス\",\"予約時間\",\"ユーザID\",\"イベントID\",\"イベントタイトル\",\"パフォーマンスID\",\"パフォーマンス名\",\"開演時間\",\"商品明細名\",\"席名\",\"商品明細金額\",\"商品明細個数\",\"手数料\",\"合計金額\",\"auth_identifier\",\"authz_identifier\",\"会員種別名\",\"会員区分名\",\"ポイント\",\"支払い方法\",\"引取方法\",\"デバイス\"\n".encode(
                'utf-8'))
        for row in orders:
            row = chop_none(row)
            file_operator.write_tmp_file(
                u"\"{0[PrimaryKey]}\",\"{0[order_no]}\",\"{0[order_status]}\",\"{0[payment_status]}\",\"{0[created_at]}\",\"{0[user_id]}\",\"{0[event_id]}\",\"{0[event_title]}\",\"{0[performance_id]}\",\"{0[performance_name]}\",\"{0[performance_start_on]}\",\"{0[product_item_name]}\",\"{0[seat_name]}\",\"{0[product_item_price]}\",\"{0[product_item_quantity]}\",\"{0[fee]}\",\"{0[total_amount]}\",\"{0[auth_identifier]}\",\"{0[authz_identifier]}\",\"{0[membership_name]}\",\"{0[membergroup_name]}\",\"{0[point]}\",\"{0[payment_method]}\",\"{0[delivery_method]}\",\"{0[user_agent]}\"\n".format(
                    row).encode('utf-8'))
        file_operator.close_tmp_file()
        file_operator.upload(old=True)
    except socket.error as e:
        logger.error(u"SPDB old upload error.{0}".format(file_operator.file_name))
    ...
    logger.info(u"SPDB old end {0}".format(file_operator.file_name))
...
def send_spdb_data(args, connection):
    ...
    try:
        ...
        orders = cur.fetchall()
        file_operator.write_tmp_file(
            u"\"PrimaryKey\",\"予約番号\",\"ステータス\",\"支払いステータス\",\"予約時間\",\"ユーザID\",\"イベントID\",\"イベントコード\",\"イベントタイトル\",\"パフォーマンスID\",\"パフォーマンス名\",\"開演時間\",\"商品明細名\",\"席名\",\"商品明細金額\",\"商品明細個数\",\"商品明細総数\",\"手数料\",\"合計金額\",\"auth_identifier\",\"authz_identifier\",\"会員種別名\",\"会員区分名\",\"ポイント\",\"支払い方法\",\"引取方法\",\"デバイス\",\"チャネル\",\"支払い方法名\",\"引取方法名\",\"枝番号\"\n".encode(
                'utf-8'))
        for row in orders:
            row = clean_data(row)
            file_operator.write_tmp_file(
                u"\"{0[PrimaryKey]}\",\"{0[order_no]}\",\"{0[order_status]}\",\"{0[payment_status]}\",\"{0[created_at]}\",\"{0[user_id]}\",\"{0[event_id]}\",\"{0[event_code]}\",\"{0[event_title]}\",\"{0[performance_id]}\",\"{0[performance_name]}\",\"{0[performance_start_on]}\",\"{0[product_item_name]}\",\"{0[seat_name]}\",\"{0[product_item_price]}\",\"1\",\"{0[product_item_quantity]}\",\"{0[fee]}\",\"{0[total_amount]}\",\"{0[auth_identifier]}\",\"{0[authz_identifier]}\",\"{0[membership_name]}\",\"{0[membergroup_name]}\",\"{0[point]}\",\"{0[payment_method]}\",\"{0[delivery_method]}\",\"{0[user_agent]}\",\"{0[channel]}\",\"{0[payment_method_name]}\",\"{0[delivery_method_name]}\",\"{0[branch_no]}\"\n".format(
                    row).encode('utf-8'))
...
def main():
    ...
    send_spdb_data_old(args, get_connection(args))
