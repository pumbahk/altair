...
spdb_sql = """
    SELECT
        ...
        CASE PaymentMethod.payment_plugin_id WHEN 1 THEN 'マルチ決済' WHEN 2 THEN '楽天ペイ' WHEN 3 THEN 'セブン' WHEN 4 THEN '窓口' WHEN 5 THEN '無料' WHEN 6 THEN 'ファミマ' WHEN 7 THEN 'クレジットカード決済_PaymentGW' ELSE '不明' END AS payment_method,
        CASE DeliveryMethod.delivery_plugin_id WHEN 1 THEN '配送' WHEN 2 THEN 'セブン' WHEN 3 THEN '窓口' WHEN 4 THEN 'QR認証' WHEN 5 THEN 'イベントゲート' WHEN 6 THEN' ファミマ' WHEN 7 THEN 'QR_AES認証' WHEN 8 THEN 'WEBクーポン引取' WHEN 9 THEN 'SKIDATA_QRゲート' WHEN 10 THEN 'GOOD_FELLOWS_QRゲート' ELSE '不明' END AS delivery_method,
        ...
        UsedDiscountCodeOrder.code as discount_code,
        DiscountCodeSetting.name as discount_code_name
    FROM
        `Order`
    ...
    LEFT JOIN DiscountCode ON DiscountCode.code = UsedDiscountCodeOrder.code
    LEFT JOIN DiscountCodeSetting ON DiscountCodeSetting.id = DiscountCode.discount_code_setting_id
