...
def includeme(config):
    ...
    for k, v in config.registry.settings.items():
        g = re.match(ur'^altair\.vissel_extauth\.member_kind_style_class\.([^.]+)', k)
        if g is not None:
            style_classes[g.group(1)] = v
    logger.debug('style_classes=%r' % style_classes)
    config.registry.registerUtility(
        VisselCommunicator(
            endpoint_base=config.registry.settings['altair.vissel_extauth.endpoint_base'],
            opener_factory=opener_factory_from_config(config, 'altair.vissel_extauth.urllib2_opener_factory'),
            client_name=config.registry.settings['altair.vissel_extauth.client_name'],
            hash_key=config.registry.settings['altair.vissel_extauth.hash_key'],
            style_classes=style_classes,
            timeout=int(config.registry.settings.get('altair.vissel_extauth.timeout', 10))
            ),
        ICommunicator,
        name='vissel'
        )
