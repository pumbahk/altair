...
class BookmarkView(BaseView):
    @view_config(route_name='bookmark.index', renderer='altair.app.ticketing:templates/bookmark/index.html')
    def index(self):
        return {
            'bookmarks' : Bookmark.find_by_organization_id(1)
        }

    @view_config(route_name='bookmark.new', request_method="GET", renderer='altair.app.ticketing:templates/bookmark/edit.html')
    def new_get(self):
        f = BookmarkForm()
        return {
    ...
    @view_config(route_name='bookmark.new', request_method="POST", renderer='altair.app.ticketing:templates/bookmark/edit.html')
    def new_post(self):
        form = BookmarkForm()
        if form.validate():
            data = form.data
    ...
    @view_config(route_name='bookmark.edit', request_method="GET", renderer='altair.app.ticketing:templates/bookmark/edit.html')
    def edit_get(self):
    ...
    @view_config(route_name='bookmark.edit', request_method="POST", renderer='altair.app.ticketing:templates/bookmark/edit.html')
    def edit_post(self):
        bookmark_id = int(self.request.matchdict.get("bookmark_id", 0))
        bookmark = Bookmark.get(bookmark_id)
        if bookmark is None:
