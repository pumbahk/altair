  ...
  % for item in order['items']:
    <div>
      ...
      <div align="right" style="text-align:right">
        ...
        金額変更理由: ${item.get('price_change_note') or u'-'}<br />
        数量変更理由: ${item.get('quantity_change_note') or u'-'}<br />
      </div>
    </div>
  % endfor
    総計<br />
    ...
    OpenID claimed ID<br />
      <div align="right" style="text-align:right">${openid_claimed_id}</div>
    利用ポイント数<br />
      <div align="right" style="text-align:right">${used_points}</div>
  </div>
