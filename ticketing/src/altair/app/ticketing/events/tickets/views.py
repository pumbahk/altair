...
def show(context, request):
    ...
    base_template_choices = [(t.id, t.name)\
                             for t in context.ticket_alls\
                                .filter(sa.or_(sa.and_(Ticket.event_id==None, Ticket.visible==True), Ticket.event_id==context.event.id))]
...
class BundleAttributeView(BaseView):
    ...
    def new_post(self):
        ...
        attr.value = self.unicodeClear(attr.value)
        attr.save()
    ...
    def unicodeClear(self, value):
        if value is None:
            return None
        return value.encode('unicode-escape').decode('ascii').replace('\u200b','').encode('ascii', 'ignore').decode('unicode_escape')
    ...
    def edit_post(self):
        ...
        attribute.value = self.unicodeClear(attribute.value)
        attribute.save()
