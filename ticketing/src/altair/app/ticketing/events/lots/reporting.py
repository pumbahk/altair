...
class LotEntryReporter(object):
    ...
    def create_report_mail(self, status):
        body = render_to_response(self.body_template,
                                  dict(lot=self.lot,
                                       between_lot_start_and_payment_due=self.lot.between_lot_start_and_payment_due(),
                                       lot_status=status))
...
def send_lot_report_mails(request, sender):
    ...
    for setting in settings:
        try:
            ...
            if cond.setting.period == ReportPeriodEnum.Entire.v[0] and not cond.is_reportable():
