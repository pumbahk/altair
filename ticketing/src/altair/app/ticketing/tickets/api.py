...
from . import VISIBLE_TICKETFORMAT_SESSION_KEY, VISIBLE_TICKETTEMPLATE_SESSION_KEY
...
def set_visible_tickettemplate(request):
    request.session[VISIBLE_TICKETTEMPLATE_SESSION_KEY] = str(datetime.now())

def set_invisible_tickettemplate(request):
    if VISIBLE_TICKETTEMPLATE_SESSION_KEY in request.session:
        del request.session[VISIBLE_TICKETTEMPLATE_SESSION_KEY]
