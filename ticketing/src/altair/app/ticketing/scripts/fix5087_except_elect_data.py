...
def except_duplicate(argv, row, wrong_writer, elect_file):
    if not except_duplicate_address(argv, row, wrong_writer, elect_file):
        return False
    if not except_duplicate_tel(argv, row, wrong_writer, elect_file):
        return False
    if not except_duplicate_mail(argv, row, wrong_writer, elect_file):
        return False
    if not except_duplicate_wrongname(argv, row, wrong_writer, elect_file):
        return False
    return True
...
def except_duplicate_address(argv, target_row, wrong_writer, elect_file):
    # 住所
...
def except_duplicate_tel(argv, target_row, wrong_writer, elect_file):
    # 電話
...
def except_duplicate_mail(argv, target_row, wrong_writer, elect_file):
    # メアド
...
def except_duplicate_wrongname(argv, target_row, wrong_writer, elect_file):
    # 同姓同名
...
def create_writer(argv, name):
    file_name = "{0}{1}.csv".format(argv[0], name)
...
def main(argv=sys.argv[1:]):
    # 抽選申込ファイルから、当選者を弾く
    # 引数：フォルダパス、結果ファイル名、抽選申込ファイル、当選リストファイル
    input_file = "{0}{1}".format(argv[0], argv[2]) # 抽選申込ファイル
    elect_file = "{0}{1}".format(argv[0], argv[3]) # 当選しているファイル

    # 入力ファイル、当選ファイルをエクセルで読めるSJIS -> UTF8へ変換
    os.system('nkf -w --overwrite {0}'.format(input_file))
    os.system('nkf -w --overwrite {0}'.format(elect_file))

    result_file_name = "{0}{1}.csv".format(argv[0], argv[1])
    ...
    file_name_list = [input_file, elect_file, result_file_name, address_file_name, tel_file_name, mail_file_name, wrongnmae_file_name]
    ...
    with open(input_file, 'r') as f:
        ...
        for row in reader:
            lot_data = LotData._make(row)
            ret = except_duplicate(argv, lot_data, wrong_writer_dict, elect_file)
