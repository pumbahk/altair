...
def _get_term_end_str(order, delivery_plugin_id):
    if not order:
        return u""
...
class CouponHelper(object):
    ...
    def get_term_end_str(order):
        return _get_term_end_str(order, RESERVE_NUMBER_DELIVERY_PLUGIN_ID)

    @staticmethod
    def get_web_coupon_term_end_str(order):
        return _get_term_end_str(order, WEB_COUPON_DELIVERY_PLUGIN_ID)
