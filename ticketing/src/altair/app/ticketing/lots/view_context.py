...
def get_lots_view_context_factory(default_package):
    ...
    class LotsViewContext(object):
        ...
        def contact_url(self):
            if self.request.organization.contact_email:
                return "mailto:%s"%self.request.organization.contact_email
            else:
                return cart_api.safe_get_contact_url(self.request)
    ...
    return LotsViewContext

def includeme(config):
    config.add_request_method(get_lots_view_context_factory(config.registry), 'view_context', reify=True)
