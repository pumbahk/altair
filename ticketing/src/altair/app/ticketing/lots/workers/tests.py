...
class send_election_mail_taskTest(unittest.TestCase, CoreTestMixin):
    def setUp(self):
        ...
        self.sales_segment_group = SalesSegmentGroup(event=self.event)
        self.organization.settings = [OrganizationSetting(cart_setting=CartSetting())]
        from ..models import Lot, LotEntry, LotEntryWish, LotElectedEntry
        self.payment_delivery_method_pairs = self._create_payment_delivery_method_pairs(self.sales_segment_group)
        self.lot = Lot(id=1)
        self.session.add(self.lot)
        self.lot_entry = LotEntry(entry_no="XX0000000000", lot=self.lot, organization_id=99,
                                  payment_delivery_method_pair=self.payment_delivery_method_pairs[0])
        self.session.add(self.lot_entry)
        self.lot_entry_wish = LotEntryWish(wish_order=1, lot_entry=self.lot_entry, performance=self.performance)
        self.session.add(self.lot_entry_wish)
        self.lot_elected_entry = LotElectedEntry(
            lot_entry=self.lot_entry,
            lot_entry_wish=self.lot_entry_wish,
            )
        self.session.add(self.lot_elected_entry)
        from altair.app.ticketing.cart.interfaces import IStocker
        from pyramid.interfaces import IRequest
        self.dummy_stocker = mock.Mock()
        self.config.registry.registerAdapter(self.dummy_stocker, [IRequest], IStocker)
    ...
    def _callFUT(self, *args, **kwargs):
        from .mails import send_election_mail_task
        return send_election_mail_task(*args, **kwargs)
    ...
    def test_it(self):
        context = testing.DummyResource(
            ...
            lot_entry=self.lot_entry,
            lot_entry_wish=self.lot_entry_wish,
            lot_elected_entry=self.lot_elected_entry,
            )
        dummy_request = DummyRequest()
        self._callFUT(context, dummy_request)


class send_rejection_mail_taskTest(unittest.TestCase, CoreTestMixin):
    def setUp(self):
        from altair.sqlahelper import register_sessionmaker_with_engine
        self.config = testing.setUp()
        self.session = _setup_db([
        ...
        self.sales_segment_group = SalesSegmentGroup(event=self.event)
        self.organization.settings = [OrganizationSetting(cart_setting=CartSetting())]
        from ..models import Lot, LotEntry, LotEntryWish, LotRejectedEntry
        self.payment_delivery_method_pairs = self._create_payment_delivery_method_pairs(self.sales_segment_group)
        self.lot = Lot(id=1)
        self.session.add(self.lot)
        self.lot_entry = LotEntry(entry_no="XX0000000000", lot=self.lot, organization_id=99,
        ...
        self.lot_entry_wish = LotEntryWish(wish_order=1, lot_entry=self.lot_entry, performance=self.performance)
        self.session.add(self.lot_entry_wish)
        self.lot_rejected_entry = LotRejectedEntry(
            lot_entry=self.lot_entry,
            )
        self.session.add(self.lot_rejected_entry)
        from altair.app.ticketing.cart.interfaces import IStocker
        from pyramid.interfaces import IRequest
        self.dummy_stocker = mock.Mock()
        self.config.registry.registerAdapter(self.dummy_stocker, [IRequest], IStocker)
    ...
    def _callFUT(self, *args, **kwargs):
        from .mails import send_rejection_mail_task
        return send_rejection_mail_task(*args, **kwargs)
    ...
    def test_it(self):
        context = testing.DummyResource(
            ...
            lot=self.lot,
            lot_entry=self.lot_entry,
            lot_rejected_entry=self.lot_rejected_entry,
            )
        dummy_request = DummyRequest()
        self._callFUT(context, dummy_request)
