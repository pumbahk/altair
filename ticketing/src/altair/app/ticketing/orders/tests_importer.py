...
class ImportCSVParserTest(unittest.TestCase, CoreTestMixin):
    ...
    def setUp(self):
        ...
        self.users = [
            User(user_credential=[UserCredential(auth_identifier=u'aho', authz_identifier=u'aho', membership=self.membership)]),
            User(user_credential=[UserCredential(auth_identifier=u'alfred', authz_identifier=u'alfred', membership=self.membership)])
            ]
        for user in self.users:
            self.session.add(user)
        self.members = [
            Member(
                auth_identifier=u'aho',
                membership=self.membership,
                membergroup=self.membergroup
                ),
            Member(
                auth_identifier=u'alfred',
                membership=self.membership,
                membergroup=self.membergroup
                ),
            ]
...
class OrderImporterTest(unittest.TestCase, CoreTestMixin):
    ...
    def setUp(self):
        ...
        self.users = [
            User(user_credential=[UserCredential(auth_identifier=u'aho', authz_identifier=u'aho', membership=self.membership)]),
            User(user_credential=[UserCredential(auth_identifier=u'aho', authz_identifier=u'alfred', membership=self.membership)]),
            ]
        for user in self.users:
            self.session.add(user)
        self.members = [
    ...
    def test_create_with_skidata_barcode(self, find_by_barcode, insert_new_data):
        ...
        self.assertEquals(len(task.proto_orders), 1)
        self.assertEquals(len(errors), 0)
        self.assertTrue(insert_new_data.called)

    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_org_setting_off(self, find_by_barcode, insert_new_data):
        """ 異常系テスト Org設定のSKIDATA連携がOFF """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.NoAutoAllocation.v, False,
                                 session=self.session)
        ...
        find_by_barcode.return_value = testing.DummyModel(id=1, ordered_product_item_token_id=None)
        self.organization.setting.enable_skidata = False
        test_event_setting = EventSetting()
        test_event_setting.enable_skidata = True
        self.event.setting = test_event_setting
        task, errors = importer(reader, self.operator, self.organization, self.performance)
        ...
        self.assertEquals(errors['XX0000000000'][0].message,
                          u'SKIDATA_QRデータはSKIDATA連携ONの場合のみ指定できます。')
        self.assertFalse(insert_new_data.called)

    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_event_setting_off(self, find_by_barcode, insert_new_data):
        """ 異常系テスト Event設定のSKIDATA連携がOFF """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.NoAutoAllocation.v, False,
                                 session=self.session)
        ...
        self.organization.setting.enable_skidata = True
        test_event_setting = EventSetting()
        test_event_setting.enable_skidata = False
        self.event.setting = test_event_setting
        task, errors = importer(reader, self.operator, self.organization, self.performance)
        self.assertEquals(len(task.proto_orders), 0)
        self.assertEquals(len(errors), 1)
    ...
    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_AlwaysAllocateNew(self, find_by_barcode, insert_new_data):
        """ 異常系テスト 配席モードが不正 """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.AlwaysAllocateNew.v, False,
                                 session=self.session)
        reader = [
            {
                u'order.order_no': u'XX0000000000',
    ...
    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_invalid_delvery_method(self, find_by_barcode, insert_new_data):
        """ 異常系テスト 引取方法が不正 """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.NoAutoAllocation.v, False,
    ...
    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_no_skidata_barcode(self, find_by_barcode, insert_new_data):
        """ 異常系テスト バーコードデータが存在しない """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        ...
        find_by_barcode.side_effect = NoResultFound
        self.organization.setting.enable_skidata = True
        test_event_setting = EventSetting()
        test_event_setting.enable_skidata = True
        self.event.setting = test_event_setting
        ...
        self.assertEquals(len(errors), 1)
        self.assertEquals(errors['XX0000000000'][0].message,
                          u'指定したSKIDATA_QRデータ「{}」は存在しません。'.format(u'test_qr_code'))
        self.assertFalse(insert_new_data.called)

    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_linked_to_order(self, find_by_barcode, insert_new_data):
        ...
        find_by_barcode.return_value = testing.DummyModel(id=1, ordered_product_item_token_id=1)
        self.organization.setting.enable_skidata = True
        test_event_setting = EventSetting()
        test_event_setting.enable_skidata = True
        self.event.setting = test_event_setting
        ...
        self.assertEquals(len(errors), 1)
        self.assertEquals(errors['XX0000000000'][0].message,
                          u'指定したSKIDATA_QRデータ「{}」はすでに予約に紐づいています。'.format(u'test_qr_code'))
        self.assertFalse(insert_new_data.called)

    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_skidata_barcode_for_multiple_ticket(self, find_by_barcode, insert_new_data):
        """ 異常系テスト 新規作成予約インポートで1QRに複数のOrderedProductItemTokenを指定した場合 """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.NoAutoAllocation.v, False,
                                 session=self.session)
        reader = [
            {
                ...
                u'ordered_product_item.product_item.name': u'product_item_of_A',
                u'ordered_product_item.price': u'10',
                u'ordered_product_item.quantity': u'2',
                u'ordered_product_item.print_histories': u'発券作業者',
                u'mail_magazine.mail_permission': u'メールマガジン受信可否',
                u'seat.name': u'Seat A-5',
                u'skidata_barcode.data': u'test_qr_code'
        ...
        self.assertEquals(len(task.proto_orders), 0)
        self.assertEquals(len(errors), 1)
        self.assertEquals(errors['XX0000000000'][0].message, u'SKIDATA_QRデータは1チケット毎に1行で指定してください。')
        self.assertFalse(insert_new_data.called)

    @mock.patch('altair.app.ticketing.orders.importer.ProtoOPIToken_SkidataBarcode.insert_new_data')
    @mock.patch('altair.app.ticketing.orders.importer.SkidataBarcode.find_by_barcode')
    def test_create_with_same_skidata_barcodes(self, find_by_barcode, insert_new_data):
        """ 異常系テスト 新規作成予約インポートで複数行に同じUTIDを紐付ける """
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Create.v, AllocationModeEnum.NoAutoAllocation.v, False,
                                 session=self.session)
        ...
        self.assertEquals(len(task.proto_orders), 1)
        self.assertEquals(len(errors), 1)
        self.assertEquals(errors['XX0000000001'][0].message,
                          u'指定したSKIDATA_QRデータ「{}」は同ファイル内で既に指定済みのため利用できません。'
                          .format(u'test_qr_code'))
        self.assertTrue(insert_new_data.called)
    ...
    def test_invalid_update_with_skidata_barcode(self):
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        from altair.app.ticketing.core.models import EventSetting
        importer = self._makeOne(self.request, ImportTypeEnum.Update.v, AllocationModeEnum.NoAutoAllocation.v, False, session=self.session)
        reader = [
        ...
        self.assertEquals(len(task.proto_orders), 0)
        self.assertEquals(len(errors), 1)
        self.assertEquals(errors['YY0000000000'][0].message,
                          u'SKIDATA_QRデータはインポート方法が「新規登録」の場合のみ指定できます。')

    def test_update_reduce_price_with_all_point_use(self):
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        existing_order_yy0000000000 = self.existing_orders[0]
        existing_order_yy0000000000.transaction_fee = 0
        existing_order_yy0000000000.point_amount = existing_order_yy0000000000.total_amount
        importer = self._makeOne(self.request, ImportTypeEnum.Update.v,
                                 AllocationModeEnum.NoAutoAllocation.v, False, session=self.session)
        reader = [
            {
                u'order.order_no': u'YY0000000000',
                u'order.status': u'ステータス',
                ...
                u'performance.code': u'ABCDEFGH',
                u'venue.name': u'会場',
                u'ordered_product.price': u'100',
                u'ordered_product.quantity': u'1',
                u'ordered_product.product.name': u'A',
                u'ordered_product.product.sales_segment.sales_segment_group.name': u'存在する販売区分グループ',
                u'ordered_product.product.sales_segment.margin_ratio': u'販売手数料率',
                u'ordered_product_item.product_item.name': u'product_item_of_A',
                u'ordered_product_item.price': u'100',
                u'ordered_product_item.quantity': u'1',
                u'ordered_product_item.print_histories': u'発券作業者',
                u'mail_magazine.mail_permission': u'メールマガジン受信可否',
                u'seat.name': u'Seat A-5',
        ...
        self.assertEquals(len(task.proto_orders), 1)
        self.assertEquals(len(errors), 1)
        _warn_msg_expected = u'全額ポイント払い予約の合計金額が290から170となったため、差額の120ポイントが' \
                             u'インポート完了後一週間以内に払戻されます'
        self.assertEquals(errors['YY0000000000'][0].message, _warn_msg_expected)

    def test_update_failure_reduce_price_less_than_min_point_with_all_point_use(self):
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum
        self.payment_delivery_method_pairs[0].special_fee = 0
        existing_order_yy0000000000 = self.existing_orders[0]
        existing_order_yy0000000000.transaction_fee = 0
        existing_order_yy0000000000.special_fee = 0
        existing_order_yy0000000000.point_amount = existing_order_yy0000000000.total_amount
        importer = self._makeOne(self.request, ImportTypeEnum.Update.v,
                                 AllocationModeEnum.NoAutoAllocation.v, False, session=self.session)
        reader = [
            {
                ...
                u'order.delivered_at': u'配送日時',
                u'order.canceled_at': u'キャンセル日時',
                u'order.total_amount': u'35',
                u'order.transaction_fee': u'0',
                u'order.delivery_fee': u'20',
                u'order.system_fee': u'10',
                u'order.special_fee': u'0',
                u'order.margin': u'内手数料金額',
                u'order.note': u'メモ',
                u'order.special_fee_name': u'特別手数料名',
                u'sej_order.billing_number': u'SEJ払込票番号',
        ...
        self.assertEquals(len(task.proto_orders), 0)
        self.assertEquals(len(errors), 1)
        _warn_msg_expected = u'ポイント利用額の下限(50)よりも合計金額を減らすことはできません'
        self.assertEquals(errors['YY0000000000'][0].message, _warn_msg_expected)

    def test_update_failure_from_all_point_use_to_partial_point_use(self):
        from altair.app.ticketing.orders.models import ImportTypeEnum, AllocationModeEnum, Order
        existing_order_yy0000000000 = self.existing_orders[0]
        existing_order_yy0000000000.transaction_fee = 0
        existing_order_yy0000000000.point_amount = existing_order_yy0000000000.total_amount
        ...
        reader = [
            {
                ...
                u'performance.code': u'ABCDEFGH',
                u'venue.name': u'会場',
                u'ordered_product.price': u'300',
                u'ordered_product.quantity': u'1',
                u'ordered_product.product.name': u'A',
                u'ordered_product.product.sales_segment.sales_segment_group.name': u'存在する販売区分グループ',
                u'ordered_product.product.sales_segment.margin_ratio': u'販売手数料率',
                u'ordered_product_item.product_item.name': u'product_item_of_A',
                u'ordered_product_item.price': u'300',
                u'ordered_product_item.quantity': u'1',
                u'ordered_product_item.print_histories': u'発券作業者',
                u'mail_magazine.mail_permission': u'メールマガジン受信可否',
                u'seat.name': u'Seat A-5',
        ...
        self.assertEquals(len(task.proto_orders), 0)
        self.assertEquals(len(errors), 1)
        _error_msg_expected = u'合計金額増額により、全額ポイント払いの予約を一部ポイント払いに変更することはできません' + \
        ...
        reader = [
            {
                ...
                u'performance.code': u'ABCDEFGH',
                u'venue.name': u'会場',
                u'ordered_product.price': u'130',
                u'ordered_product.quantity': u'1',
                u'ordered_product.product.name': u'A',
                u'ordered_product.product.sales_segment.sales_segment_group.name': u'存在する販売区分グループ',
                u'ordered_product.product.sales_segment.margin_ratio': u'販売手数料率',
                u'ordered_product_item.product_item.name': u'product_item_of_A',
                u'ordered_product_item.price': u'130',
                u'ordered_product_item.quantity': u'1',
                u'ordered_product_item.print_histories': u'発券作業者',
                u'mail_magazine.mail_permission': u'メールマガジン受信可否',
                u'seat.name': u'Seat A-5',
