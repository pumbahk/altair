    ...
    def release(self):
        ...
        for seat_status in self.seat_statuses_for_update:
            ...
            if seat_status.status not in cancellable_status:
                ...
                raise InvalidStockStateError("Order %s is associated with a seat (id=%d, status=%d) that is not marked ordered" % (self.orderd_product.order_no, seat_status.seat_id, seat_status.status))
            else:
