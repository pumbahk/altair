...
class FamiPortOrderAutoCopleterTest(TestCase):
    ...
    @mock.patch('altair.app.ticketing.famiport.scripts.famiport_auto_complete.FamiPortOrderAutoCompleter._fetch_target_famiport_orders')
    @mock.patch('altair.app.ticketing.famiport.scripts.famiport_auto_complete.FamiPortOrderAutoCompleter._do_complete')
    def test_complete(self, _do_complete, _fetch_target_famiport_orders):
        from ..famiport_auto_complete import AutoCompleterStatus
        session = mock.Mock()
        famiport_orders = [mock.Mock() for ii in range(10)]
        _fetch_target_famiport_orders.return_value = famiport_orders
        target = self._get_target(session)
        res = target.complete()
        ...
        self.assertEqual(do_complete_call_args_list, famiport_orders)
        self.assertEqual(session_call_args_list, famiport_orders)
