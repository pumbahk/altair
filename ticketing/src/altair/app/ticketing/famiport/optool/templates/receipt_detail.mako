...
<div id="table-content">
  ...
  <table class="table table-hover">
    ...
    <tbody>
      ...
      <tr>
        <th>管理番号</th>
        <td>${vh.format_famiport_order_identifier(receipt.famiport_order.famiport_order_identifier)}</td>
        <th>発券区分</th>
  ...
  success: function (result) {
    $.each(result.pages, function (_, pageUrl) {
      ...
      if (xhr.status == 500) {
          ...
          if (responseJSON != null && responseJSON.status == 'invalid_encode') {
              $viewport.append("券面に使用できない文字が使われています。（使用不可文字：" + responseJSON.message + ")");
          } else {
              $viewport.append('エラーが発生しました');
          }
      } else {
          $viewport.append('エラーが発生しました');
      }
