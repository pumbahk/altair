...
from altair.app.ticketing.famiport.mdm.shop_code_change import shop_code_change_schema
from altair.app.ticketing.famiport.mdm.shop_master import shop_master_schema
...
class ShopMasterSchemaTest(unittest.TestCase):
    ...
    def setUp(self):
        self.schema = shop_master_schema
        self.marshalled = u'00000,1,0000,1,北海道ファミリーマート,1,01,1,北海道,1,20150101,1,001,1,北海道,1,20150101,1,' \
                          u'札幌北1条,1,サッポロキタイチジョウ,1,0112523938,1,1,1,北海道,1,札幌市中央区北1条東1丁目4-1,' \
                          u'1,20150201,1,060-0031,1,20150130,1,0000,1,2359,1,2400,1,1,1,札幌,1,1,1,1,1,,0,,0,,0,,0,' \
                          u'35,0,135,0,1,1,1,1,20150201,1,20150201,1,,1,,1,,1,,1,1,01,0,0'
        self.unmarshalled = {
    ...
    def test_it(self):
        self.assertEqual(marshal(self.schema, self.unmarshalled), self.marshalled)
...
class ShopCodeChangeSchemaTest(unittest.TestCase):
    def setUp(self):
        ...
        self.unmarshalled = {
            'management_number': u'002287854',  # 管理番号,
            'type': 2,  # 処理区分
            'processed_at': datetime(2015, 9, 7, 19, 29, 27),  # 処理日時
            'valid': True,  # 赤黒区分
            'shop_code': u'0070874',  # 実店番
        }
...
def marshal(schema, unmarshalled):
    f = BytesIO()
    m = make_marshaller(f, schema)
    m(unmarshalled)
    return six.text_type(f.getvalue().rstrip(), 'CP932')


def unmarshal(schema, marshalled):
    f = BytesIO()
    f.write(marshalled.encode('CP932'))
    f.seek(0)
    m = make_unmarshaller(f, schema)
    return m.next()
