var deform  = {
    ...
    addSequenceItem: function (protonode, before) {
        ...
        var namematch = /(.+)?-[#]{3}/;
        var html = decodeURIComponent(protonode.attr('prototype'));
        var $htmlnode = $(html);
        ...
        $htmlnode.attr("id", anchorid).insertBefore(before);
    ...
    appendSequenceItem: function(node) {
        ...
        var $before_node = $oid_node.find('.deformInsertBefore');
    ...
    processSequenceButtons: function(oid_node, min_len, max_len, now_len) {
        var closeButton = oid_node.find('.deformClosebutton');
        closeButton.removeClass('deformClosebuttonActive');
        oid_node.children('.deformSeqAdd').show();
        if (now_len > min_len) {
            closeButton.addClass('deformClosebuttonActive');
        };
        if (now_len >= max_len) {
