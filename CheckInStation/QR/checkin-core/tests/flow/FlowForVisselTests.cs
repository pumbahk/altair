...
namespace checkin.core
{
    ...
    [TestFixture()]
	public class FlowForVisselTests
    {
		//認証後すぐにorderno読み込み
		[Test, Description ("認証input画面 -ok-> password入力 -ok> auth情報取得 -ok-> orderno読み込み")]
        public void TestAuthFlow ()
        {            
            var resource = new Resource ();

            var manager = new FlowManager (new VisselFlowDefinition ());
        ...
        // 1枚発券。まとめて発券選ばない
		[Test,Description ("vissel: QR読み込み -ok-> QRからデータ取得中 -ok-> 印刷(one) -ok> 発券しました -ok-> orderno読み込み")]
		public void TestItForQR__Vissel ()
        {
            var manager = new FlowManager (new VisselFlowDefinition());
        ...
        public void TestItForOrderno_Vissel ()
        {
