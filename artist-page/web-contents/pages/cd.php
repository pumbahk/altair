...
$photo_url = isset($_GET['photo'])?$_GET['photo']:null;
if(!photo_url){
header("Status:404");
...
$stmt_photo_url = $dbh->prepare("select playList,salesDate,largephoto,cdname,id  from cds where photo = ?");
$stmt_photo_url->bind_param('s', $photo_url);
$stmt_photo_url->execute();
$stmt_photo_url->bind_result($playList, $salesDate, $largephoto,$cdname,$id);
$stmt_photo_url->fetch();
$stmt_photo_url->close();
return compact('playList', 'salesDate', 'largephoto','cdname','id');
}
$cds_info = playlist_limg_saldate($dbh,$photo_url);
$cds_id = $cds_info['id'];

function artist_name($dbh,$cds_id){
$stmt_artist_detail = $dbh->prepare("select  artist.name,artist.yomigana,artist.prof from cd_artist inner  join artist on artist.id = cd_artist.artist_id inner join cds on cds.id = cd_artist.cds_id where cds_id = ?");
$stmt_artist_detail->bind_param('s',$cds_id);
$stmt_artist_detail->execute();
$stmt_artist_detail->bind_result($name,$yomigana,$prof);
if (!$stmt_artist_detail->fetch())
	return null;
$stmt_artist_detail->close();
return compact('name','yomigana','prof');
}


$cds_info = playlist_limg_saldate($dbh,$photo_url);
$artist_info = array();
$artist_info= artist_name($dbh,$cds_info['id']);
